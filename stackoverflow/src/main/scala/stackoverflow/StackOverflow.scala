...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] =
    ...
    val type1Posts = postings.filter(_.postingType == 1) // questions
    val type2Posts = postings.filter(_.postingType == 2) // answers
    val questionsKeyed = type1Posts.map(post => (post.id, post))    // _2 == id
    val answersKeyed = type2Posts.map(post => (post.parentId.get, post))  // _4 == parentId
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    grouped.map { case (id, entries) =>
      val a = entries.map(p => p._2).toArray
      val q = entries.head._1
      val s = answerHighScore(a)
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] =
    ...
    (scored.flatMap { (posting, score) =>
      firstLangInTag(posting.tags, langs) match
        case Some(langIdx) => (langIdx * langSpread, score) :: Nil
        case None => Nil
    }).persist()
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val newMeansMap: collection.Map[(Int, Int), (Int, Int)] =
      vectors
        .groupBy(p => findClosest(p, means))
        .mapValues(ps => averageVectors(ps))
        .collectAsMap()
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val valuesList = vs.toList
      val clusterSize = valuesList.length
      val langsGrouped = valuesList.groupBy(elem => elem._1).toList
      val (langValue, langQuestions) = langsGrouped.sortBy(-_._2.size).head
      val langPercent = 100 * langQuestions.size.toDouble / langsGrouped.foldLeft(0.0)((acc, elt) => acc + elt._2.size)
      val langLabel = langs(langValue / langSpread)
      val answerScores = valuesList.map(v => v._2).sorted
      val medianScore =
        if clusterSize % 2 == 0 then
          (answerScores(clusterSize / 2 - 1) + answerScores(clusterSize / 2)) / 2
        else
          answerScores(clusterSize / 2)
